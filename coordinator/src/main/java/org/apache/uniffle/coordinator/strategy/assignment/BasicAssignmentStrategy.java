...
public class BasicAssignmentStrategy extends AbstractAssignmentStrategy {
  ...
  public PartitionRangeAssignment assign(int totalPartitionNum, int partitionNumPerRange,
      ...
      int requiredShuffleServerNumber, int estimateTaskConcurrency,
    ...
    List<ServerNode> servers = getRequiredServers(requiredTags, expectedShuffleNodesNum, excludeServerNodes);
  ...
  public PartitionRangeAssignment assign(int totalPartitionNum, int partitionNumPerRange,
    ...
    List<ServerNode> servers = getRequiredServers(requiredTags, expectedShuffleNodesNum, Collections.EMPTY_SET);
    if (servers.isEmpty() || servers.size() < replica) {
      return new PartitionRangeAssignment(null);
    }
   ...
   * @return
   */
  private List<ServerNode> getRequiredServers(Set<String> requiredTags, int expectedNum, Set<String> excludeServerInfos) {
