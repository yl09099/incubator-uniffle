          ...
          if (status.getPartitionFetchFailureNum(stageAttempt, partitionId, shuffleManager)) {
            ...
            if (!status.isClearedMapTrackerBlock()) {
              try {
                ...
                LOG.info(
                    "Clear shuffle result in shuffleId:{}, stageId:{} in the read failure phase.",
                    shuffleId,
                    stageAttempt);
              } catch (SparkException e) {
                LOG.error(
                    "Clear MapoutTracker Meta failed in shuffleId:{}, stageAttemptId:{} in the read failure phase.",
                    shuffleId,
                    stageAttempt);
                throw new RssException("Clear MapoutTracker Meta failed!", e);
              }
