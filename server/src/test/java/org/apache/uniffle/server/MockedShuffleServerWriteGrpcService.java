...
public class MockedShuffleServerWriteGrpcService extends ShuffleServerGrpcService {
  private static final Logger LOG = LoggerFactory.getLogger(MockedShuffleServerWriteGrpcService.class);
  /**
   * Abnormal node flag
   */
  ...
  @Override
  public void sendShuffleData(RssProtos.SendShuffleDataRequest req,
                              StreamObserver<RssProtos.SendShuffleDataResponse> responseObserver) {
    if(taskExcptionFlag) {
      failedWriteRequest.incrementAndGet();
      throw new RuntimeException("This write request is failed as mocked failure,failed times: "+ failedWriteRequest.get());
    }
